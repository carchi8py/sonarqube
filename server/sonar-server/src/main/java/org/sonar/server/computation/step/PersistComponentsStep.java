...
public class PersistComponentsStep implements ComputationStep {
  ...
  private ComponentDto processComponent(ComponentContext componentContext, BatchReport.Component reportComponent, @Nullable ComponentDto moduleParent){
    ...
    ComponentDto componentDto = componentContext.componentDtosByKey.get(componentKey);
    if (componentDto == null) {
      if (reportComponent.getType().equals(Constants.ComponentType.PROJECT) || reportComponent.getType().equals(Constants.ComponentType.MODULE)) {
        componentDto = persistModule(componentContext, reportComponent, componentKey, moduleParent);
      } else if (moduleParent != null) {
        componentDto = persistNoneModule(componentContext, reportComponent, componentKey, moduleParent);
      }
    }
    return componentDto;
  }

  private ComponentDto persistModule(ComponentContext componentContext, BatchReport.Component reportComponent, String componentKey, @Nullable ComponentDto parentModule) {
    ...
    if (parentModule == null) {
      component.setModuleUuidPath(ComponentDto.MODULE_UUID_PATH_SEP + uuid + ComponentDto.MODULE_UUID_PATH_SEP);
      component.setProjectUuid(uuid);
    } else {
      component.setParentProjectId(parentModule.getId());
      component.setModuleUuid(parentModule.uuid());
      component.setModuleUuidPath(parentModule.moduleUuidPath() + uuid + ComponentDto.MODULE_UUID_PATH_SEP);
  ...
  private ComponentDto persistNoneModule(ComponentContext componentContext, BatchReport.Component reportComponent, String componentKey, ComponentDto parentModule) {
    ComponentDto component = new ComponentDto();
    String uuid = Uuids.create();
    component.setUuid(uuid);
    component.setKey(componentKey);
    ...
    component.setParentProjectId(parentModule.getId());
    component.setModuleUuid(parentModule.uuid());
    component.setModuleUuidPath(parentModule.moduleUuidPath());
    component.setProjectUuid(parentModule.projectUuid());
  ...
  private static String getScope(BatchReport.Component reportComponent){
    switch (reportComponent.getType()){
      case PROJECT: return Scopes.PROJECT;
      case MODULE: return Scopes.PROJECT;
      case DIRECTORY: return Scopes.DIRECTORY;
      case FILE: return Scopes.FILE;
    }
  ...
  private static String getQualifier(BatchReport.Component reportComponent){
    switch (reportComponent.getType()){
      case PROJECT: return Qualifiers.PROJECT;
      case MODULE: return Qualifiers.MODULE;
      case DIRECTORY: return Qualifiers.DIRECTORY;
      case FILE: return !reportComponent.getIsTest() ? Qualifiers.FILE : Qualifiers.UNIT_TEST_FILE;
    }
    throw new IllegalArgumentException(String.format("Unknown type '%s'", reportComponent.getType()));
  }
